...
export function projectReductionLayout(frame, width, height) {
...
export function drawReductionFrame(canvas, frame) {
  ...
  const layout = projectReductionLayout(frame, width, height);
  ...
  if (frame.kind === "cpu") {
    ...
    layout.inputNodes.forEach((node) => circle(context, node.x, node.y, 12, COLORS.background, COLORS.muted, node.value));
    layout.nodes.forEach((node) => circle(context, node.x, node.y, layout.nodeRadius, node.active ? COLORS.greenSoft : COLORS.background, node.active ? COLORS.green : COLORS.muted, node.active ? node.value : "×"));
    context.fillStyle = COLORS.muted; context.textAlign = "center"; context.fillText(frame.round === 0 ? "VALUES WAITING FOR WORKERS" : layout.nodes.length > 1 ? "PAIR RESULTS · SAME ROUND" : "FINAL SUM", width / 2, height * 0.82);
  }
}
